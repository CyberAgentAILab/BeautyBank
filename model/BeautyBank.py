...
class BeautyBank(nn.Module):
    ...
    def forward(
        self,
        styles, # bare-face style code
        makeups, # makeup style code
        ...
        z_plus_latent=False, # bare-face style code is z+ or z
        ...
        if use_res:
            if makeups.ndim < 3:
                resstyles = self.style(makeups).unsqueeze(1).repeat(1, self.generator.n_latent, 1)
                adastyles = makeups.unsqueeze(1).repeat(1, self.generator.n_latent, 1)
            else:
                nB, nL, nD = makeups.shape
                resstyles = self.style(makeups.reshape(nB*nL, nD)).reshape(nB, nL, nD)
                adastyles = makeups
